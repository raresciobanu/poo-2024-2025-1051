...
ostream& operator<<(ostream& out, const Profesor& p) {
	out << (Persoana)p;//a refolosi op << din parinte
	out << "Profesor vechime: " << p.aniVechime << endl;

	return out;
}

istream& operator>>(istream& in, Profesor& p) {
	in >> (Persoana&)p;//a refolosi op >> din parinte
	cout << "Profesor vechime: "; in >> p.aniVechime;
...
ostream& operator<<(ostream& out, const Student& s) {
	out << (Persoana)s;
	out << "Studentul are: " << s.aniStudii << " ani de studii si este exmatriculat: "
		<< (s.exmatriculat ? " DA " : " NU ") << endl;
	out << "Nr note: " << s.nrNote << endl;
	for (int i = 0; i < s.nrNote; i++) {
		out << "nota[" << i << "] = " << s.note[i] << " ";
	}
	out << endl << "Nivel studii: " << s.nivelStudii << endl;
...
istream& operator>>(istream& in, Student& s) {
	...
	cout << "Ani studii: "; in >> s.aniStudii;

	//in.ignore(); //se foloseste doar cand apelam in.getline()

	char aux[100];
	cout << "Este exmatriculat: "; in >> aux;
	if (strcmp(aux, "DA") == 0) {
		s.exmatriculat = true;
	}
	else {
		s.exmatriculat = false;
	}

	cout << "Nr note: "; in >> s.nrNote;

	delete[] s.note; //eviti memory leaks
	s.note = new double[s.nrNote];
	for (int i = 0; i < s.nrNote; i++) {
		cout << "nota[" << i << "] = "; in >> s.note[i];
	}

	//in.ignore(); //se foloseste doar cand apelam in.getline()

	cout << "Nivel studii: "; in >> aux;

	delete[] s.nivelStudii; //eviti memory leaks
	s.nivelStudii = new char[strlen(aux) + 1];
	strcpy_s(s.nivelStudii, strlen(aux) + 1, aux);
...
ostream& operator<<(ostream& out, const Curs& c) {
	out << "Curs nume: " << c.nume << endl;

	out << "PROFESOR: " << c.profesor << endl;

	out << "Nr studenti: " << c.nrStudenti << ". Studentii: " << endl;
...
istream& operator>>(istream& in, Curs& c) {
	...
	cout << "Nr studenti: ";  in >> c.nrStudenti;

	delete[] c.studenti; //evitare memory leaks
	c.studenti = new Student[c.nrStudenti];
